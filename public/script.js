...
const apiBase = "http://localhost:5005"; /** API BASE URL */
...
async function fetchTodos() {
  ...
  try {
    // Fetch todos from databases
    const response = await fetch(apiBase + "/todos", {
      headers: { Authorization: token },
    });

    // Check if response is JSON and contains a valid array
    const todosData = await response.json();
    console.log("Todos from API:", todosData);

    // If the response is not an array, throw an error
    if (!Array.isArray(todosData)) {
      throw Error("Invalid response format: Expected an array");
    }
    ...
    textError.innerHTML = `Failed to load todos: ${error.message}`;
